...
- 返回按钮和 history 栈行为
- **外链点击与跳转限制**：
  - 是否存在诱导分享、诱导下载或骚扰性外链（易被封禁）。
  - 跳转外链是否会触发“如需浏览，请长按网址复制后使用浏览器访问”或中间提示页。
  - 针对文件下载、App Store 跳转或第三方 App 唤起的处理（微信常有拦截）。
  - 域名是否在微信黑名单或未备案，导致无法直接打开。
...
- **合规化外链行为**：
  - 避免使用触发诱导机制的文案和逻辑。
  - 对于下载类需求，引导用户点击右上角“在浏览器中打开”。
  - 优先使用官方推荐的跳转方式（如开放标签 `wx-open-launch-app`）。
  - 确保域名备案并符合微信外部链接内容管理规范。
